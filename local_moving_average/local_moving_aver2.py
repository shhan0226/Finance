...
stock_data_filtered = stock_data.loc[output_date_str:end_date]
...
for i in range(len(stock_data_filtered)):
    ...
    #######################################
    if row['60D_MA'] > row['120D_MA'] and row['20D_MA'] > row['120D_MA']:
        if row['20D_MA'] > row['60D_MA'] and (position is None or position == 'sell'):
            buy_signals.append((row.name, row['Close']))
            position = 'buy'
            shares = cash / row['Close']
            cash = 0
            trades.append(('Buy', row.name, row['Close']))
        elif row['20D_MA'] < row['60D_MA'] and position == 'buy':
            sell_signals.append((row.name, row['Close']))
            position = 'sell'
            cash = shares * row['Close']
            shares = 0
            trades.append(('Sell', row.name, row['Close']))
    # elif row['60D_MA'] < row['120D_MA']:
    #     if row['20D_MA'] > row['120D_MA'] and (position is None or position == 'sell'):
    #         buy_signals.append((row.name, row['Close']))
    #         position = 'buy'
    #         shares = cash / row['Close']
    #         cash = 0
    #         trades.append(('Buy', row.name, row['Close']))
    #     elif row['20D_MA'] < row['120D_MA'] and position == 'buy':
    #         sell_signals.append((row.name, row['Close']))
    #         position = 'sell'
    #         cash = shares * row['Close']
    #         shares = 0
    ...
    #         cash = 0
    #         trades.append(('Buy', row.name, row['Close']))
    #     elif row['20D_MA'] > row['Close'] and (position == 'buy'):
    #         sell_signals.append((row.name, row['Close']))
    #         position = 'sell'
    #         cash = shares * row['Close']
    #         shares = 0
